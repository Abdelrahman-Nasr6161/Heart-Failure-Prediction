...
print("Tuning Random Forest Tree Hyperparameters...")
start = time.time()
for  num_of_classifiers, depth, min_samples, n_components in itertools.product(num_of_classifiers_vals, depths, min_samples_split_vals, n_components_vals):
    ...
    bdt = RandomForest(num_of_classifiers=num_of_classifiers,max_depth=depth, min_samples_split=min_samples)
...
final_model = RandomForest(num_of_classifiers=best_num_of_classifiers,max_depth=best_depth, min_samples_split=best_min_samples)
...
result_dir = f'results/random_forest/num_of_classifiers_{best_num_of_classifiers}_depth_{best_depth}_min_{best_min_samples}'
...
plt.title(f'Random Forest Confusion Matrix (Depth {best_depth})')
